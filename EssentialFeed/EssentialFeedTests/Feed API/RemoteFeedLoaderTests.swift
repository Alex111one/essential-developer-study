...
class RemoteFeedLoaderTests: XCTestCase {
    ...
    func test_load_deliversErrorOnClientError() {
        ...
        expect(sut, toCompleteWith: .failure(
            RemoteFeedLoader.Error.connectivity), when: {
    ...
    func test_load_deliversErrorOnNon200HTTPResponse() {
        ...
        samples.enumerated().forEach { index, code in
            expect(sut, toCompleteWith: .failure(
                RemoteFeedLoader.Error.invalidData), when: {
    ...
    private func trackForMemoryLeaks(_ instance: AnyObject,
        ...
        addTeardownBlock { [weak instance] in
            XCTAssertNil(instance, "Should have been deallocated. Memory leak",
            file: file, line: line)
        }
    }
    
    private func makeItem(
        id: UUID, description: String? = nil, location: String? = nil,
    ...
    private func expect(
        ...
        sut.load { receivedResult in
            ...
            default:
                XCTFail("No match \(expectedResult) \(receivedResult)",
                file: file, line: line)
            }
            expectation.fulfill()
        }
